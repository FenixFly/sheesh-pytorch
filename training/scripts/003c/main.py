...
def main(opt=None, cv_config=None, split=None):
    ...
    data_transform = mytransforms.Compose([
        ...
        mytransforms.RandomCrop(transform_keys=['signal','label','time','x'], size=(np.nan,128)),
        mytransforms.PadToShape(transform_keys=['signal','label','time','x'], shape=(np.nan,128),mode='reflect'),
    ...
    optimizer = torch.optim.AdamW(arch.parameters(), lr=1e-3, weight_decay=1e-3,amsgrad=True)
    schedulers = {'scheduler': lr_scheduler.OneCycleLR(optimizer=optimizer,max_lr=2e-3,total_steps=int(opt.nEpochs*len(train_data)/opt.batchSize)),#lr_scheduler.MultiStepLR(optimizer=optimizer,milestones=[50,80],gamma=0.2),
                  'name': 'step_scheduler',
                  'interval': 'step',#'epoch',
                  }
