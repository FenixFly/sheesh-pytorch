...
if __name__ == '__main__':
    ...
    for idx, f in enumerate(split_idx):
        ...
        for batch in evaluation_data_loader:
            ...
            pad_left = pad_left.cpu().numpy()[0]
            pad_right = pad_right.cpu().numpy()[0]
            output_crop = loader_helper.revert_pad(output, (pad_left, pad_right), dim=[1])[0]
            time = loader_helper.revert_pad(time, (pad_left, pad_right), dim=[1])[0]
            x = loader_helper.revert_pad(x, (pad_left, pad_right), dim=[1])[0]
            y = loader_helper.revert_pad(y, (pad_left, pad_right), dim=[1])[0]

            scan_result = pd.DataFrame(data={'id':batch['id'][0].item(),
                               'time':time.astype(np.int),'x':x,'y':y,'y_pred':output_crop,'split':idx})
    ...
    df_results.to_csv('cv_results_c12.csv')
